...
import { 
  ...
  History, DollarSign, MinusCircle, UserPlus, ShieldAlert, Ticket, Send, Menu 
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  return (
    <div className="flex h-screen bg-[#F5F7FB] font-sans overflow-hidden flex-col md:flex-row">
      ...
      {/* SIDEBAR */}
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-slate-300 flex flex-col shadow-2xl transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 border-b border-slate-800 shrink-0 flex items-center justify-between">
          <h1 className="text-xl font-bold text-white flex items-center gap-2">
            <Activity className="w-6 h-6 text-blue-500" /> Admin
          </h1>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
          ...
          <button onClick={() => { setActiveTab('overview'); setIsMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${activeTab === 'overview' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'hover:bg-slate-800'}`}>
            <LayoutDashboard className="w-5 h-5" /> Tổng quan
          </button>
          <button onClick={() => { setActiveTab('users'); setIsMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${activeTab === 'users' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'hover:bg-slate-800'}`}>
            <Users className="w-5 h-5" /> Khách hàng
          </button>
          <button onClick={() => { setActiveTab('packages'); setIsMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${activeTab === 'packages' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'hover:bg-slate-800'}`}>
          ...
          <button onClick={() => { setActiveTab('approvals'); setIsMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${activeTab === 'approvals' ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/20' : 'hover:bg-slate-800'}`}>
            <Clock className="w-5 h-5" /> Duyệt Rút tiền
          </button>
          <button onClick={() => { setActiveTab('deposits'); setIsMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${activeTab === 'deposits' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'hover:bg-slate-800'}`}>
            <ArrowDownToLine className="w-5 h-5" /> Lịch sử Nạp (Mua gói)
          </button>
          <button onClick={() => { setActiveTab('withdrawals'); setIsMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${activeTab === 'withdrawals' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'hover:bg-slate-800'}`}>
          ...
          <button onClick={() => { setActiveTab('events'); setIsMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${activeTab === 'events' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'hover:bg-slate-800'}`}>
            <Calendar className="w-5 h-5" /> Sự kiện
          </button>
          <button onClick={() => { setActiveTab('leaderboards'); setIsMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${activeTab === 'leaderboards' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'hover:bg-slate-800'}`}>
            <Trophy className="w-5 h-5" /> Đua top
          </button>
          <button onClick={() => { setActiveTab('giftcodes'); setIsMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${activeTab === 'giftcodes' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'hover:bg-slate-800'}`}>
            <Ticket className="w-5 h-5" /> Quản lý Mã Code
          </button>
          <button onClick={() => { setActiveTab('settings'); setIsMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${activeTab === 'settings' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'hover:bg-slate-800'}`}>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        {/* MOBILE HEADER */}
        <header className="md:hidden h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 shrink-0 z-30">
            <div className="flex items-center gap-3">
               <Activity className="w-6 h-6 text-blue-600" />
               <span className="font-bold text-slate-800">Admin Panel</span>
            </div>
            <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 md:mb-8">
                ...
                {activeTab === 'packages' && (
                    <button onClick={openAddPackage} className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 md:py-2.5 rounded-xl font-bold flex justify-center items-center gap-2 shadow-sm transition-all">
                        <Plus className="w-5 h-5" /> Thêm Gói Mới
                    </button>
                )}
                {activeTab === 'events' && (
                    <button onClick={openAddEvent} className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 md:py-2.5 rounded-xl font-bold flex justify-center items-center gap-2 shadow-sm transition-all">
                        <Plus className="w-5 h-5" /> Thêm Sự Kiện
                    </button>
                )}
                {activeTab === 'leaderboards' && (
                    <button onClick={openAddLb} className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 md:py-2.5 rounded-xl font-bold flex justify-center items-center gap-2 shadow-sm transition-all">
                        <Plus className="w-5 h-5" /> Thêm Người Đua Top
                    </button>
                )}
                {activeTab === 'giftcodes' && (
                    <button onClick={openAddGiftcode} className="w-full md:w-auto bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-3 md:py-2.5 rounded-xl font-bold flex justify-center items-center gap-2 shadow-sm transition-all">
            ...
            {activeTab === 'overview' && (
                ...
                <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200 mb-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
                        <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
                            <div className="w-12 h-12 rounded-full bg-blue-100 text-blue-600 flex shrink-0 items-center justify-center"><Users className="w-6 h-6"/></div>
                            <div>
                                <p className="text-xs md:text-sm font-bold text-slate-500 uppercase">Tổng Tài khoản</p>
                                <h3 className="text-xl md:text-2xl font-black text-slate-800">{users.length}</h3>
                            </div>
                        </div>
                        <div className="flex items-center gap-4 bg-emerald-50 p-4 rounded-xl border border-emerald-100">
                            <div className="w-12 h-12 rounded-full bg-emerald-100 text-emerald-600 flex shrink-0 items-center justify-center"><UserPlus className="w-6 h-6"/></div>
                            <div>
                                <p className="text-xs md:text-sm font-bold text-emerald-700 uppercase">Đăng ký Hôm nay</p>
                                <h3 className="text-xl md:text-2xl font-black text-emerald-700">+{newUsersTodayCount}</h3>
                            </div>
                        </div>
                        <div className="flex items-center gap-4 bg-rose-50 p-4 rounded-xl border border-rose-100">
                            <div className="w-12 h-12 rounded-full bg-rose-100 text-rose-600 flex shrink-0 items-center justify-center"><ShieldAlert className="w-6 h-6"/></div>
                            <div>
                                <p className="text-xs md:text-sm font-bold text-rose-700 uppercase">Bị Khóa / Chặn</p>
                                <h3 className="text-xl md:text-2xl font-black text-rose-700">{lockedUsersCount}</h3>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                    <div className="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-200">
                        <p className="text-xs md:text-sm font-bold text-slate-500 uppercase">Khách Nạp Hôm Nay</p>
                        <h3 className="text-xl md:text-2xl font-black text-emerald-600 mt-2">+{todayDeposits.toLocaleString('vi-VN')} ₫</h3>
                    </div>
                    <div className="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-200">
                        <p className="text-xs md:text-sm font-bold text-slate-500 uppercase">Khách Rút Hôm Nay</p>
                        <h3 className="text-xl md:text-2xl font-black text-rose-600 mt-2">-{todayWithdrawals.toLocaleString('vi-VN')} ₫</h3>
                    </div>
                    <div className="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-200">
                        <p className="text-xs md:text-sm font-bold text-slate-500 uppercase">Tổng Nạp (All Time)</p>
                        <h3 className="text-xl md:text-2xl font-black text-emerald-600 mt-2">+{totalDeposits.toLocaleString('vi-VN')} ₫</h3>
                    </div>
                    <div className="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-200">
                        <p className="text-xs md:text-sm font-bold text-slate-500 uppercase">Tổng Rút (All Time)</p>
                        <h3 className="text-xl md:text-2xl font-black text-rose-600 mt-2">-{totalWithdrawals.toLocaleString('vi-VN')} ₫</h3>
                    </div>
                </div>
                ...
                <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200 mt-6 mb-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                        <div className="border border-emerald-100 bg-emerald-50/30 rounded-xl p-4 md:p-5">
                            <p className="font-bold text-emerald-700 mb-3 text-center uppercase tracking-wide text-sm md:text-base">Khách Đã Mua Gói</p>
                            <div className="flex justify-between items-center text-sm">
                                <div className="text-center w-1/2 border-r border-emerald-200/50">
                                    <div className="text-slate-500 mb-1 text-xs md:text-sm">Hôm nay</div>
                                    <div className="font-black text-xl md:text-2xl text-emerald-600">{totalUsersBoughtToday}</div>
                                </div>
                                <div className="text-center w-1/2">
                                    <div className="text-slate-500 mb-1 text-xs md:text-sm">Tất cả thời gian</div>
                                    <div className="font-black text-xl md:text-2xl text-emerald-600">{totalUsersBoughtAllTime}</div>
                                </div>
                            </div>
                        </div>

                        <div className="border border-slate-200 bg-slate-50/50 rounded-xl p-4 md:p-5">
                            <p className="font-bold text-slate-700 mb-3 text-center uppercase tracking-wide text-sm md:text-base">Khách Chưa Mua Gói</p>
                            <div className="flex justify-between items-center text-sm">
                                <div className="text-center w-1/2 border-r border-slate-200">
                                    <div className="text-slate-500 mb-1 text-xs md:text-sm">Hôm nay (TK mới)</div>
                                    <div className="font-black text-xl md:text-2xl text-slate-600">{totalUsersNotBoughtToday}</div>
                                </div>
                                <div className="text-center w-1/2">
                                    <div className="text-slate-500 mb-1 text-xs md:text-sm">Tất cả thời gian</div>
                                    <div className="font-black text-xl md:text-2xl text-slate-600">{totalUsersNotBoughtAllTime}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                  ...
                  <div className="p-4 border-b border-slate-200 bg-white">
                      <div className="relative w-full md:max-w-md">
                          ...
                          <input
                              ...
                              className="w-full pl-10 pr-4 py-3 md:py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50 focus:bg-white transition-colors text-sm font-medium"
                          />
                      </div>
                  </div>
                    ...
                    <table className="w-full text-left border-collapse whitespace-nowrap md:whitespace-normal">
                      <thead>
                        <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-sm">
                          <th className="p-4 font-semibold min-w-[200px]">Tài khoản Đăng ký</th>
                          ...
                          <th className="p-4 font-semibold text-center sticky right-0 bg-slate-50 shadow-[-4px_0_10px_rgba(0,0,0,0.02)]">Thao tác</th>
                        </tr>
                      </thead>
                      <tbody className="text-sm">
                        ...
                        {processedUsers.map(u => (
                          <tr key={u.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                            ...
                            <td className="p-4 text-center sticky right-0 bg-white shadow-[-4px_0_10px_rgba(0,0,0,0.02)] z-10">
                                <button 
                                ...
                                {openDropdownId === u.id && (
                                    <>
                                        ...
                                        <div className="absolute right-0 top-12 w-56 md:w-60 bg-white border border-slate-200 rounded-xl shadow-2xl z-[70] overflow-hidden py-1 origin-top-right">
                                            <button onClick={() => { openEditUser(u); setOpenDropdownId(null); }} className="w-full text-left px-4 py-3 md:py-2.5 text-sm hover:bg-slate-50 flex items-center gap-3 text-slate-700 font-medium transition-colors">
                                                <Edit className="w-4 h-4 text-slate-400"/> Sửa thông tin NH
                                            </button>
                                            <div className="h-px bg-slate-100 my-1"></div>
                                            <button onClick={() => { openUserTxs(u); setOpenDropdownId(null); }} className="w-full text-left px-4 py-3 md:py-2.5 text-sm hover:bg-slate-50 flex items-center gap-3 text-slate-700 font-medium transition-colors">
                                                <History className="w-4 h-4 text-slate-400"/> Xem lịch sử giao dịch
                                            </button>
                                            <button onClick={() => { openManageUserPackages(u); setOpenDropdownId(null); }} className="w-full text-left px-4 py-3 md:py-2.5 text-sm hover:bg-slate-50 flex items-center gap-3 text-slate-700 font-medium transition-colors">
                                                <Package className="w-4 h-4 text-slate-400"/> Quản lý gói (Thêm/Xóa)
                                            </button>
                                            <div className="h-px bg-slate-100 my-1"></div>
                                            <button onClick={() => { openAddMoney(u); setOpenDropdownId(null); }} className="w-full text-left px-4 py-3 md:py-2.5 text-sm hover:bg-emerald-50 flex items-center gap-3 text-emerald-600 font-bold transition-colors">
                                                <DollarSign className="w-4 h-4"/> Cộng tiền vào TK
                                            </button>
                                            <button onClick={() => { openDeductMoney(u); setOpenDropdownId(null); }} className="w-full text-left px-4 py-3 md:py-2.5 text-sm hover:bg-rose-50 flex items-center gap-3 text-rose-600 font-bold transition-colors">
                  ...
                  <table className="w-full text-left border-collapse whitespace-nowrap md:whitespace-normal">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-sm">
                        ...
                        <th className="p-4 font-semibold text-center sticky right-0 bg-slate-50">Thao tác</th>
                      </tr>
                    </thead>
                    <tbody className="text-sm">
                      ...
                      {packages.map(p => (
                        <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                          ...
                          <td className="p-4">
                              {p.badge ? <span className="bg-[#1E6EFF] text-white text-xs px-2 py-1 rounded font-bold uppercase">{p.badge}</span> : <span className="text-slate-400 italic text-xs">Không</span>}
                          </td>
                          <td className="p-4 text-center sticky right-0 bg-white shadow-[-4px_0_10px_rgba(0,0,0,0.02)]">
                            <div className="flex justify-center gap-2">
                                <button onClick={() => openEditPackage(p)} className="p-2 md:px-3 md:py-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 font-medium transition-colors">
                                  <Edit className="w-4 h-4 md:mr-1 inline" /> <span className="hidden md:inline">Sửa</span>
                                </button>
                                <button onClick={() => handleDeletePackage(p.id)} className="p-2 md:px-3 md:py-1.5 bg-rose-50 text-rose-600 rounded-lg hover:bg-rose-100 font-medium transition-colors">
                                  <Trash2 className="w-4 h-4 md:mr-1 inline" /> <span className="hidden md:inline">Xóa</span>
                                </button>
                            </div>
                          </td>
                        </tr>
                  ...
                  <table className="w-full text-left border-collapse whitespace-nowrap md:whitespace-normal">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-sm">
                        ...
                        <th className="p-4 font-semibold">Thưởng (VNĐ)</th>
                    ...
                    <tbody className="text-sm">
                      ...
                      {giftcodes.map(gc => (
                        <tr key={gc.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                          ...
                          <td className="p-4 text-center sticky right-0 bg-white shadow-[-4px_0_10px_rgba(0,0,0,0.02)]">
                            <div className="flex justify-center gap-2">
                                <button onClick={() => openEditGiftcode(gc)} className="p-2 md:px-3 md:py-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 font-medium transition-colors">
                                  <Edit className="w-4 h-4 md:mr-1 inline" /> <span className="hidden md:inline">Sửa</span>
                                </button>
                                <button onClick={() => handleDeleteGiftcode(gc.id)} className="p-2 md:px-3 md:py-1.5 bg-rose-50 text-rose-600 rounded-lg hover:bg-rose-100 font-medium transition-colors">
                                  <Trash2 className="w-4 h-4 md:mr-1 inline" /> <span className="hidden md:inline">Xóa</span>
                                </button>
                            </div>
                          </td>
                    ...
                    <tbody className="text-sm">
                      ...
                      {events.map(evt => (
                        <tr key={evt.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                          <td className="p-4 text-center">
                              {evt.image_url ? (
                                  <img src={evt.image_url} alt={evt.title} className="w-12 h-12 md:w-16 md:h-16 object-cover rounded-lg border border-slate-200 shadow-sm mx-auto" />
                              ) : (
                                  <div className="w-12 h-12 md:w-16 md:h-16 bg-slate-100 rounded-lg border border-slate-200 flex items-center justify-center text-slate-400 mx-auto">
                                      <ImageIcon className="w-5 h-5 md:w-6 md:h-6 opacity-50" />
                                  </div>
                              )}
                          </td>
                          <td className="p-4 font-bold text-slate-800 max-w-[150px] md:max-w-[200px] truncate">{evt.title}</td>
                          <td className="p-4 text-slate-600 font-medium truncate">{evt.date}</td>
                          <td className="p-4">
                          ...
                          <td className="p-4 text-center sticky right-0 bg-white shadow-[-4px_0_10px_rgba(0,0,0,0.02)]">
                            <div className="flex justify-center gap-2">
                                <button onClick={() => openEditEvent(evt)} className="p-2 md:px-3 md:py-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 font-medium transition-colors">
                                  <Edit className="w-4 h-4 md:mr-1 inline" /> <span className="hidden md:inline">Sửa</span>
                                </button>
                                <button onClick={() => handleDeleteEvent(evt.id)} className="p-2 md:px-3 md:py-1.5 bg-rose-50 text-rose-600 rounded-lg hover:bg-rose-100 font-medium transition-colors">
                                  <Trash2 className="w-4 h-4 md:mr-1 inline" /> <span className="hidden md:inline">Xóa</span>
                                </button>
                            </div>
                          </td>
                  ...
                  <table className="w-full text-left border-collapse whitespace-nowrap md:whitespace-normal">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-sm">
                        ...
                        <th className="p-4 font-semibold text-center">Lượt mời</th>
                        <th className="p-4 font-semibold text-right">Phần thưởng</th>
                        <th className="p-4 font-semibold text-center sticky right-0 bg-slate-50">Thao tác</th>
                      </tr>
                    </thead>
                    <tbody className="text-sm">
                      {leaderboards.length === 0 && <tr><td colSpan={5} className="p-8 text-center text-slate-500">Chưa có người dùng nào trên bảng xếp hạng.</td></tr>}
                      {leaderboards.map((lb, idx) => {
                        ...
                        return (
                          <tr key={lb.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                            ...
                            <td className="p-4 font-bold text-slate-800">{lb.name}</td>
                            ...
                            <td className="p-4 text-center sticky right-0 bg-white shadow-[-4px_0_10px_rgba(0,0,0,0.02)]">
                              <div className="flex justify-center gap-2">
                                  <button onClick={() => openEditLb(lb)} className="p-2 md:px-3 md:py-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 font-medium transition-colors">
                                    <Edit className="w-4 h-4 md:mr-1 inline" /> <span className="hidden md:inline">Sửa</span>
                                  </button>
                                  <button onClick={() => handleDeleteLb(lb.id)} className="p-2 md:px-3 md:py-1.5 bg-rose-50 text-rose-600 rounded-lg hover:bg-rose-100 font-medium transition-colors">
                                    <Trash2 className="w-4 h-4 md:mr-1 inline" /> <span className="hidden md:inline">Xóa</span>
                                  </button>
                              </div>
                            </td>
                  ...
                  <table className="w-full text-left border-collapse whitespace-nowrap md:whitespace-normal">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-sm">
                        ...
                        <th className="p-4 font-semibold">Thông tin STK nhận (Chạm vào QR)</th>
                        <th className="p-4 font-semibold text-rose-600 text-right">Số tiền rút (VNĐ)</th>
                        <th className="p-4 font-semibold text-center sticky right-0 bg-slate-50">Xử lý</th>
                      </tr>
                    </thead>
                    <tbody className="text-sm">
                            ...
                            <div className="flex items-center gap-3">
                                {t.bank_name && t.bank_account && (
                                    <div className="shrink-0 relative group">
                                        ...
                                        />
                                        <div className="absolute left-full ml-3 top-1/2 -translate-y-1/2 hidden group-active:block group-hover:block z-50">
                                            <img 
                                                src={`https://img.vietqr.io/image/${getBankCode(t.bank_name)}-${t.bank_account.trim()}-qr_only.png?amount=${t.amount || 0}&addInfo=Thanh toan rut tien&accountName=${encodeURIComponent(t.account_name || '')}`}
                                                alt="QR Zoom"
                                                className="w-48 h-48 md:w-56 md:h-56 max-w-none object-contain bg-white border border-slate-200 rounded-2xl p-3 shadow-2xl"
                                            />
                                        </div>
                                    </div>
                                )}
                                <div className="min-w-[120px]">
                                    ...
                                    <div className="text-slate-600 font-medium text-xs uppercase truncate">{t.account_name || 'Chưa cập nhật Tên CTK'}</div>
                                    <div className="text-slate-500 font-mono mt-0.5">{t.bank_account || 'Không có STK'}</div>
                                </div>
                            </div>
                          </td>
                          <td className="p-4 font-extrabold text-rose-600 text-base text-right">{t.amount?.toLocaleString('vi-VN')} ₫</td>
                          <td className="p-4 text-center sticky right-0 bg-white shadow-[-4px_0_10px_rgba(0,0,0,0.02)]">
                            <div className="flex flex-col md:flex-row justify-center gap-2">
                                <button onClick={() => handleApproveWithdrawal(t)} title="Duyệt thành công" className="flex items-center justify-center gap-1 px-3 py-2 bg-emerald-100 text-emerald-700 rounded-lg hover:bg-emerald-200 font-bold transition-colors">
                                    <Check className="w-4 h-4" /> <span className="hidden md:inline">Duyệt</span>
                                </button>
                                <button onClick={() => handleRejectWithdrawal(t)} title="Từ chối lệnh" className="flex items-center justify-center gap-1 px-3 py-2 bg-rose-100 text-rose-700 rounded-lg hover:bg-rose-200 font-bold transition-colors">
                                    <X className="w-4 h-4" /> <span className="hidden md:inline">Từ chối</span>
                                </button>
                                <button onClick={() => handleSuspendWithdrawal(t.id)} title="Treo lệnh (Ẩn đi)" className="flex items-center justify-center gap-1 px-3 py-2 bg-amber-100 text-amber-700 rounded-lg hover:bg-amber-200 font-bold transition-colors">
                                    <EyeOff className="w-4 h-4" /> <span className="hidden md:inline">Treo</span>
                                </button>
                            </div>
                          </td>
                        </tr>
                    ...
                    <tbody className="text-sm">
                      {transactions.filter(t => t.type === 'rut_tien' && t.status !== 'pending').map(t => (
                        <tr key={t.id} className="border-b border-slate-100 hover:bg-slate-50">
                          ...
                          <td className="p-4 font-bold text-rose-600 text-right">-{t.amount?.toLocaleString('vi-VN')} ₫</td>
                          <td className="p-4 text-center">
                              <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                                  ...
                                  {t.status === 'success' ? 'Thành công' : t.status === 'rejected' ? 'Từ chối' : 'Treo'}
                              </span>
                          </td>
                        </tr>
                      ))}
                ...
                <div className="max-w-2xl space-y-6 pb-10">
                    <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200">
                        <h3 className="font-bold text-xl text-slate-800 mb-6 flex items-center gap-2">
                            <SettingsIcon className="w-6 h-6 text-blue-600" /> Cấu hình Hỗ trợ (CSKH)
                        </h3>
                        <div className="mb-6">
                            <label className="block text-sm font-bold text-slate-700 mb-2">Link Zalo / Telegram</label>
                            <input 
                                type="text" 
                                placeholder="VD: https://zalo.me/0123456789" 
                                value={cskhLink} 
                                onChange={e => setCskhLink(e.target.value)} 
                                className="w-full border border-slate-200 bg-slate-50 focus:bg-white rounded-xl p-4 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-medium text-slate-800 text-sm" 
                            />
                        </div>
                        <button onClick={saveSettingsInfo} className="w-full md:w-auto px-8 py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg transition-all">
                            Lưu Cấu Hình
                        </button>
                    </div>
                    ...
                    <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200">
                        <h3 className="font-bold text-xl text-slate-800 mb-4 flex items-center gap-2">
                            <Send className="w-6 h-6 text-[#229ED9]" /> Kiểm tra Bot Telegram
                        </h3>
                        <p className="text-sm text-slate-500 mb-6">
                            Bấm để gửi một tin nhắn test đến Telegram của bạn để kiểm tra kết nối API.
                        </p>
                        
                        <button 
                            onClick={handleTestTelegram} 
                            disabled={isTestingTele}
                            className="w-full md:w-auto px-8 py-3.5 bg-[#229ED9] hover:bg-[#1e8ec2] text-white font-bold rounded-xl shadow-lg transition-all disabled:opacity-70 flex justify-center items-center gap-2"
                        >
      ...
      {/* TẤT CẢ CÁC MODALS (Bo góc, giới hạn chiều cao cho mobile) */}
      {isEditUserOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white p-4 md:p-6 rounded-2xl w-full max-w-sm shadow-2xl relative max-h-[90vh] overflow-y-auto">
                <button onClick={() => setIsEditUserOpen(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-700">
                    <X className="w-6 h-6" />
                </button>
                <h3 className="font-bold text-xl text-slate-900 mb-6">Sửa Ngân hàng</h3>
                <div className="mb-4">
                    <label className="block text-sm font-bold text-slate-700 mb-2">Ngân hàng</label>
                    <input type="text" value={newBankName} onChange={e => setNewBankName(e.target.value)} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div className="mb-4">
                    <label className="block text-sm font-bold text-slate-700 mb-2">Số tài khoản</label>
                    <input type="text" value={newBankAccount} onChange={e => setNewBankAccount(e.target.value)} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div className="mb-8">
                    <label className="block text-sm font-bold text-slate-700 mb-2">Tên CTK</label>
                    <input type="text" value={newAccountName} onChange={e => setNewAccountName(e.target.value.toUpperCase())} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <button onClick={saveUserBankInfo} className="w-full py-3.5 bg-blue-600 text-white font-bold rounded-xl shadow-lg transition-all">Lưu Thay Đổi</button>
            </div>
        </div>
      )}
      ...
      {isPackageModalOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white p-4 md:p-6 rounded-2xl w-full max-w-md shadow-2xl relative max-h-[90vh] overflow-y-auto">
                ...
                <h3 className="font-bold text-xl text-slate-900 mb-6">{editingPackage ? 'Sửa Gói' : 'Thêm Gói'}</h3>
                <div className="space-y-4 mb-8">
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Tên Gói</label>
                        <input type="text" value={pkgForm.name} onChange={e => setPkgForm({...pkgForm, name: e.target.value})} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Lợi nhuận (%)</label>
                        <input type="text" value={pkgForm.return_rate} onChange={e => setPkgForm({...pkgForm, return_rate: e.target.value})} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Mức giá</label>
                        <input type="text" value={pkgForm.limits} onChange={e => setPkgForm({...pkgForm, limits: e.target.value})} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Thời gian (Duration)</label>
                        <input type="text" value={pkgForm.duration} onChange={e => setPkgForm({...pkgForm, duration: e.target.value})} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Tag nổi bật (Badge)</label>
                        <input type="text" value={pkgForm.badge} onChange={e => setPkgForm({...pkgForm, badge: e.target.value})} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                </div>
                <button onClick={savePackageInfo} className="w-full py-3.5 bg-blue-600 text-white font-bold rounded-xl shadow-lg transition-all">{editingPackage ? 'Lưu Thay Đổi' : 'Xác nhận'}</button>
            </div>
        </div>
      )}
      ...
      {isGiftcodeModalOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white p-4 md:p-6 rounded-2xl w-full max-w-md shadow-2xl relative max-h-[90vh] overflow-y-auto">
                ...
                <h3 className="font-bold text-xl text-slate-900 mb-6">{editingGiftcode ? 'Sửa Mã Code' : 'Thêm Mã Code'}</h3>
                <div className="space-y-4 mb-8">
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Mã Code (Tự viết hoa)</label>
                        <input type="text" value={gcForm.code} onChange={e => setGcForm({...gcForm, code: e.target.value.toUpperCase()})} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono font-bold tracking-widest" />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Giá trị thưởng (VNĐ)</label>
                        <input type="number" value={gcForm.reward_amount} onChange={e => setGcForm({...gcForm, reward_amount: e.target.value})} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Số lượt dùng tối đa</label>
                        <input type="number" value={gcForm.usage_limit} onChange={e => setGcForm({...gcForm, usage_limit: e.target.value})} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                ...
                <button onClick={saveGiftcodeInfo} className="w-full py-3.5 bg-emerald-600 text-white font-bold rounded-xl shadow-lg transition-all">{editingGiftcode ? 'Lưu Thay Đổi' : 'Tạo Mã Code'}</button>
            </div>
        </div>
      )}
      ...
      {isEventModalOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white p-4 md:p-6 rounded-2xl w-full max-w-md shadow-2xl relative max-h-[90vh] overflow-y-auto">
                ...
                <h3 className="font-bold text-xl text-slate-900 mb-6">{editingEvent ? 'Sửa Sự kiện' : 'Thêm Sự kiện'}</h3>
                ...
                <div className="space-y-4 mb-8">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center gap-4">
                            {eventForm.image_url ? (
                                <div className="relative self-start md:self-auto">
                                    ...
                                    <button onClick={() => setEventForm({...eventForm, image_url: ''})} className="absolute -top-2 -right-2 bg-rose-500 text-white rounded-full p-1 hover:bg-rose-600">
                                        <X className="w-3 h-3" />
                                    </button>
                                </div>
                            ) : (
                                <div className="w-20 h-20 bg-slate-100 rounded-xl border border-slate-200 border-dashed flex flex-col items-center justify-center text-slate-400 shrink-0">
                            ...
                            <label className="flex-1 cursor-pointer w-full">
                                <div className={`w-full border border-slate-200 bg-slate-50 hover:bg-slate-100 rounded-xl p-3 flex items-center justify-center gap-2 transition-colors ${isUploadingImg ? 'opacity-50 cursor-not-allowed' : ''}`}>
                                    ...
                                    <span className="font-medium text-slate-700 text-sm whitespace-nowrap">
                                        {isUploadingImg ? 'Đang tải...' : 'Chọn ảnh...'}
                                    </span>
                                </div>
                    ...
                    <div>
                        ...
                        <input type="text" value={eventForm.title} onChange={e => setEventForm({...eventForm, title: e.target.value})} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Thời gian</label>
                        <input type="text" value={eventForm.date} onChange={e => setEventForm({...eventForm, date: e.target.value})} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Nội dung</label>
                        <textarea value={eventForm.content} onChange={e => setEventForm({...eventForm, content: e.target.value})} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[100px]" />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Trạng thái</label>
                ...
                <button onClick={saveEventInfo} className="w-full py-3.5 bg-blue-600 text-white font-bold rounded-xl shadow-lg transition-all">{editingEvent ? 'Lưu Thay Đổi' : 'Tạo Sự Kiện'}</button>
            </div>
        </div>
      )}
      ...
      {isLbModalOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white p-4 md:p-6 rounded-2xl w-full max-w-md shadow-2xl relative max-h-[90vh] overflow-y-auto">
                ...
                <h3 className="font-bold text-xl text-slate-900 mb-6">{editingLb ? 'Sửa Đua top' : 'Thêm Người Đua Top'}</h3>
                <div className="space-y-4 mb-8">
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Tên hiển thị</label>
                        <input type="text" value={lbForm.name} onChange={e => setLbForm({...lbForm, name: e.target.value})} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Số lượt mời</label>
                        <input type="number" value={lbForm.invites} onChange={e => setLbForm({...lbForm, invites: Number(e.target.value)})} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Phần thưởng (Nếu có)</label>
                        <input type="text" value={lbForm.reward} onChange={e => setLbForm({...lbForm, reward: e.target.value})} className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                </div>
                <button onClick={saveLbInfo} className="w-full py-3.5 bg-blue-600 text-white font-bold rounded-xl shadow-lg transition-all">{editingLb ? 'Lưu Thay Đổi' : 'Xác nhận'}</button>
            </div>
        </div>
      )}
      ...
      {isTxsModalOpen && selectedUserTxs && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white p-4 md:p-6 rounded-2xl w-full max-w-3xl shadow-2xl relative max-h-[90vh] overflow-hidden flex flex-col">
                ...
                <p className="text-sm text-slate-500 mb-4 border-b border-slate-200 pb-4 truncate">Tài khoản: <span className="font-bold text-blue-600">{getUserEmail(selectedUserTxs.id)}</span></p>
                ...
                <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-2 mb-4">
                    ...
                    <button onClick={() => setTxFilter('thu_hoi')} className={`px-4 py-2 rounded-lg text-sm font-bold whitespace-nowrap transition-colors ${txFilter === 'thu_hoi' ? 'bg-orange-600 text-white' : 'bg-orange-50 text-orange-700 hover:bg-orange-100'}`}>Thu hồi</button>
                </div>

                <div className="flex-1 overflow-x-auto overflow-y-auto pr-0 md:pr-2 bg-slate-50 rounded-xl border border-slate-200">
                                        ...
                                        <tr key={`${t.id}-${idx}`} className="border-b border-slate-100 bg-white hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="p-3">
                                                <span className={`text-xs font-bold ${t.status === 'success' ? 'text-emerald-600' : t.status === 'rejected' ? 'text-rose-600' : t.status === 'pending' ? 'text-amber-600' : 'text-slate-500'}`}>
                                                    {t.status === 'success' ? 'Thành công' : t.status === 'pending' ? 'Đang xử lý' : t.status === 'rejected' ? 'Từ chối' : 'Lỗi'}
                                                </span>
                                            </td>
      ...
      {isManagePkgModalOpen && managePkgUser && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white p-4 md:p-6 rounded-2xl w-full max-w-xl shadow-2xl relative max-h-[90vh] overflow-y-auto">
                ...
                <h3 className="font-bold text-xl text-slate-900 mb-1">Quản lý Gói</h3>
                <p className="text-sm text-slate-500 mb-6 border-b border-slate-200 pb-4 truncate">Tài khoản: <span className="font-bold text-blue-600">{getUserEmail(managePkgUser.id)}</span></p>
                ...
                <div className="space-y-3 mb-8">
                    {userPackages.filter(p => p.user_id === managePkgUser.id).length === 0 ? (
                        <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl text-center text-slate-500 text-sm">Khách chưa có gói nào.</div>
                    ) : (
                        userPackages.filter(p => p.user_id === managePkgUser.id).map(p => (
                            <div key={p.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 border border-slate-200 rounded-xl bg-slate-50 gap-3">
                                <div>
                                    <div className="font-bold text-slate-800">{p.package_name}</div>
                                    <div className="text-xs text-slate-500 mt-1">Vốn: <span className="font-semibold text-slate-700">{p.invested_amount?.toLocaleString('vi-VN')}đ</span><br className="sm:hidden" /> Mua lúc: {new Date(p.purchased_at || p.created_at).toLocaleDateString('vi-VN')}</div>
                                </div>
                                <button onClick={() => handleRemovePackageFromUser(p.id)} className="w-full sm:w-auto p-2 text-rose-500 bg-rose-100 hover:bg-rose-200 rounded-lg transition-colors text-sm font-bold flex items-center justify-center gap-1" title="Xóa gói này">
                                    <Trash2 className="w-4 h-4" /> <span className="sm:hidden">Xóa gói</span>
                                </button>
                            </div>
                        ))
                    )}
                ...
                <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 md:p-5">
                    <h4 className="font-bold text-blue-800 mb-4 flex items-center gap-2"><Plus className="w-4 h-4"/> Thêm gói mới</h4>
                    <div className="space-y-4">
                        <div>
                            <label className="block text-xs font-bold text-blue-800 mb-1">Chọn gói hệ thống</label>
                            <select value={selectedNewPkgId} onChange={(e) => setSelectedNewPkgId(e.target.value)} className="w-full border border-blue-200 bg-white rounded-lg p-3 md:p-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full border border-blue-200 bg-white rounded-lg p-3 md:p-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                            />
                        </div>
                        <button onClick={handleAddPackageToUser} className="w-full py-3.5 md:py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg transition-colors text-sm">
                            Xác Nhận Thêm
                        </button>
                    </div>
                </div>
      ...
      {isAddMoneyModalOpen && addMoneyUser && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white p-4 md:p-6 rounded-2xl w-full max-w-sm shadow-2xl relative max-h-[90vh] overflow-y-auto">
                ...
                <p className="text-sm text-slate-500 mb-6 truncate">TK: <span className="font-bold text-blue-600">{getUserEmail(addMoneyUser.id)}</span></p>
                ...
                <div className="mb-2">
                    ...
                    <input 
                        ...
                        className="w-full border border-slate-200 bg-slate-50 rounded-xl p-4 focus:outline-none focus:ring-2 focus:ring-emerald-500 font-bold text-lg text-emerald-600" 
                    />
                </div>
                <div className="mb-6 text-xs text-slate-500">
                    * Sẽ cộng trực tiếp tiền vào số dư và ghi nhận "Nạp tiền" thành công vào lịch sử.
                </div>
                
                <button onClick={handleAddMoney} className="w-full py-3.5 bg-emerald-600 text-white font-bold rounded-xl shadow-lg transition-all">
                    Xác Nhận Cộng
                </button>
            </div>
        </div>
      ...
      {isDeductMoneyModalOpen && deductMoneyUser && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white p-4 md:p-6 rounded-2xl w-full max-w-sm shadow-2xl relative border-t-4 border-rose-500 max-h-[90vh] overflow-y-auto">
                ...
                <p className="text-sm text-slate-500 mb-4 truncate">TK: <span className="font-bold text-blue-600">{getUserEmail(deductMoneyUser.id)}</span></p>
                ...
                <div className="mb-2">
                    ...
                    <input 
                        ...
                        className="w-full border border-slate-200 bg-slate-50 rounded-xl p-4 focus:outline-none focus:ring-2 focus:ring-rose-500 font-bold text-lg text-rose-600" 
                    />
                </div>
                <div className="mb-6 text-xs text-slate-500">
                    * Sẽ trừ trực tiếp tiền từ số dư. Lịch sử ghi nhận là giao dịch "Thu hồi/Trừ tiền".
                </div>
                
                <button onClick={handleDeductMoney} className="w-full py-3.5 bg-rose-600 text-white font-bold rounded-xl shadow-lg transition-all">
                    Xác Nhận Trừ Tiền
                </button>
            </div>
        </div>
