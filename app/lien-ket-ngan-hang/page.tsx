...
export default function LinkBankPage() {
  ...
  const [accountName, setAccountName] = useState<string>(''); // THÊM STATE TÊN CHỦ TÀI KHOẢN
  ...
  useEffect(() => {
    const fetchCurrentBankInfo = async () => {
      ...
      // Lấy thông tin ngân hàng hiện tại nếu đã liên kết (thêm account_name)
      const { data: profile } = await supabase
        .from('profiles')
        .select('bank_name, bank_account, account_name')
        .eq('id', session.user.id)
        .single();

      if (profile) {
        if (profile.bank_account) setBankAccount(profile.bank_account);
        if (profile.account_name) setAccountName(profile.account_name); // Gán tên CTK nếu có
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!accountName.trim()) {
      setMessage({ text: 'Vui lòng nhập tên chủ tài khoản!', type: 'error' });
      setIsSaving(false);
      return;
    }
    ...
    try {
      const { error } = await supabase
        ...
        .update({ 
          ...
          account_name: accountName.trim().toUpperCase() // LƯU VÀO DB VÀ ÉP VIẾT HOA
        })
        .eq('id', userId);
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="p-6 md:p-8">
          <form onSubmit={handleSave} className="space-y-6">
            ...
            {/* Ô TÊN CHỦ TÀI KHOẢN (MỚI) */}
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                <User className="w-4 h-4 text-[#1E6EFF]" /> Tên Chủ Tài Khoản
              </label>
              <input 
                type="text" 
                value={accountName}
                ...
                className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3.5 focus:outline-none focus:ring-2 focus:ring-[#1E6EFF] transition-all font-bold text-slate-800 tracking-wide uppercase"
              />
            </div>
